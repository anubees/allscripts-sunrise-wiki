...
namespace Roadkill.Core.Plugins.Text.BuiltIn
{
	...
	public class EmoticonsPlugin : TextPlugin
	{
        ...
        private string TranslateCorrectNotation(string html)
        {
            ...
            //E.g: [{:)}]
            const string regexCorrect = @"(?<!\[)\{(\:[()DP]|\;\)|(\(([ynix!+\-?*\/]|on|off|h|c|iron|coke|u|z|p|oa|nxt|e1|e2|cancel|calc|ppb|ppp|ppg|pplb|cldy|\$|s|sr|gf|@|@[rgby])\)))\}(?!\])";
        ...
        private static string TranslateEscapedNotations(string html)
        {
            ...
            //E.g: [{:)}]
            const string regexEscape = @"\[\{(\:[()DP]|\;\)|(\(([ynix!+\-?*\/]|on|off|h|c|iron|coke|u|z|p|oa|nxt|e1|e2|cancel|calc|ppb|ppp|ppg|pplb|cldy|\$|s|sr|gf|@|@[rgby])\)))\}\]";
